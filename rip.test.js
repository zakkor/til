...
test('does not remove used type selectors', () => {
	...
	const r = ripData(html, css, false)
	expect(r).toEqual(['<style>a{padding:4px}</style><a> hi </a>'])
})
...
test('does not remove used class selectors', () => {
	...
	const r = ripData(html, css, false)
	expect(r).toEqual(['<style>.used{padding:4px}</style><a class="used"> hi </a>'])
})
...
test('does not remove used id selectors', () => {
	...
	const r = ripData(html, css, false)
	expect(r).toEqual(['<style>#used{padding:4px}</style><a id="used"> hi </a>'])
})
...
test('removes unused type selectors', () => {
	...
	const r = ripData(html, css, false)
	expect(r).toEqual(['<style></style><a> hi </a>'])
})
